var polycrypt = (function polycrypt() {
    ...
    var me = {},  // members of this object are appended to self.crypto
        nextOpId = 0,  // a UID for each object this function instantiates

    // vars for dealing with the polycrypt backend
        backendSource = "http://polycrypt.net/back/back.html",
        backendOrigin = "http://polycrypt.net",
        backendFrame,
        backend = null,
        handleAlive;
    ...
    function loadBackendFrame() {
        ...
        self.backendFrame = backendFrame;
    }
    self.addEventListener("load", loadBackendFrame);
    ...
    var createCryptoOp = function pc_createCryptoOp(
        ...
        // Register for events coming back
        self.addEventListener('message', messenger._handleMessage);
    ...
    handleAlive = function crypto_handleAlive(e) {
        if ('polycrypt backend is alive' === e.data) {
            ...
            self.removeEventListener('message', crypto_handleAlive, false);
        }
    };

    self.addEventListener('message', handleAlive, false);
    //--------------------------------------------------------------------------
    return me;
}());