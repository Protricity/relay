...
(function() {
    ...
    importScripts('server/socket/templates/sockets-log-template.js');
    ...
    Sockets.get = function(socketURL) {
        ...
        function onOpen(e) {
            ...
            Templates.socket.log.action("SOCKET OPEN: " + newSocket.url, function(html) {
                Client.postResponseToClient('RENDER.APPEND socket-log:' + newSocket.url + ' ' + html);
            });
...
//             console.info("I " + e.data);
            ...
            if(socket instanceof WebSocket) {
                Templates.socket.log.entry(e.data, 'I', function(html) {
                    Client.postResponseToClient('RENDER.APPEND socket-log:' + socket.url + ' ' + html);
                });
            }
        }
        ...
        Templates.socket.log.container(newSocket.url, function(html) {
            Client.postResponseToClient('RENDER socket:' + newSocket.url + ' ' + html);
        });
